...
import { checkRateLimit, getRateLimitKey, RATE_LIMITS, logSuspiciousActivity, detectAnomalousBalanceChange } from '@/lib/rate-limit'
...
export async function POST(request: Request) {
  try {
    ...
    // Check rate limit for gift attempts
    const rateLimitKey = getRateLimitKey(user.id, 'gift')
    const rateLimitResult = checkRateLimit(rateLimitKey, RATE_LIMITS.GIFT)

    if (!rateLimitResult.success) {
      logSuspiciousActivity(user.id, 'gift', 'Rate limit exceeded', {
      ...
      return NextResponse.json(
        {
          error: 'Too many gift attempts. Please wait before trying again.',
    ...
    // Check for anomalous activity (large gifts)
    if (amount > 500) {
      logSuspiciousActivity(user.id, 'gift', 'Large gift amount', {
        amount,
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      // Check for anomalous balance changes on recipient
      if (detectAnomalousBalanceChange(recipient.chronos, recipientNewBalance, 'gift')) {
        logSuspiciousActivity(recipient.id, 'gift_received', 'Anomalous balance change detected', {
          oldBalance: recipient.chronos,
