...
export async function GET(request: NextRequest) {
  try {
    ...
    // Age-gating: Filter personas by age bracket and NSFW for minors
    const filteredPersonas = personas.filter(p => {
      // Filter by age bracket
      const ownerDob = p.user?.dateOfBirth ?? null
      if (!canInteract(userDob, ownerDob)) return false
      
      // Filter out NSFW personas for minors
      if (isUserMinor && p.nsfwEnabled) return false
      
      return true
    })
    
    const result = filteredPersonas.map(p => ({
      id: p.id,
      name: p.name,
