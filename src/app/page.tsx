        ...
        {!personasLoading && personas.length === 0 && (
          <div className="persona-card persona-card-hover mb-6 p-4">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-teal-500/20 to-cyan-500/20 flex items-center justify-center border border-white/[0.08]">
        ...
        <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <select
              ...
              className="bg-[#0f1117] border border-white/[0.08] rounded-lg px-3 py-1.5 text-sm text-slate-300 focus:outline-none focus:ring-2 focus:ring-teal-500/25 appearance-none cursor-pointer hover:border-teal-500/25 transition-colors"
              style={{
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm transition-all ${
              showOffline 
                ? 'bg-teal-500/15 border border-teal-500/25 text-slate-200' 
                : 'bg-white/[0.03] border border-white/[0.08] text-slate-400 hover:border-teal-500/25'
            }`}
          >
            <span className={`w-2 h-2 rounded-full ${showOffline ? 'bg-teal-400' : 'bg-emerald-400 animate-pulse'}`} />
            {showOffline ? 'All Users' : 'Online Only'}
          </button>
        </div>
        
        <p className="text-sm text-slate-500 mb-4 flex items-center gap-2 flex-wrap">
          <span className={`w-2 h-2 rounded-full flex-shrink-0 ${showOffline ? 'bg-teal-400' : 'bg-emerald-400 animate-pulse'}`} />
          <span>{showOffline ? 'Showing all personas (online and offline)' : 'Showing personas that are currently online'}</span>
          {archetypeFilter !== 'all' && (
            <span className="text-teal-400 text-xs">• Filtered: {archetypeFilter}</span>
          )}
        </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {onlinePersonas.slice(0, 3).map((persona, index) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 rounded-xl bg-gradient-to-br from-amber-500/15 via-transparent to-teal-500/15 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                  <div className="relative bg-[#0f1117] rounded-xl border border-amber-500/15 group-hover:border-amber-500/30 overflow-hidden">
            ...
            {[...Array(10)].map((_, i) => (
              <div key={i} className="persona-card overflow-hidden animate-pulse">
                <div className="pt-4 pb-2 flex justify-center">
                  <div className="w-16 h-16 rounded-full bg-teal-500/15" />
                </div>
                <div className="px-3 pb-3 text-center">
                  <div className="h-4 bg-teal-500/15 rounded w-3/4 mx-auto" />
...
function MainApp() {
  ...
  // Navigation mode: 'static' (sidebar) or 'linear' (topbar + DM sidebar)
    ...
    <div className="h-screen w-full overflow-hidden bg-gradient-to-b from-[#0b0d11] via-[#0f1117] to-[#0b0d11] text-slate-100">
      <div className="flex h-full">
        {navigationMode === 'static' ? (
          ...
          /* Linear mode: DM Sidebar takes Sidebar's place on the left */
          <DMSidebar
        ...
        {/* Linear mode: Show NavigationTopbar at top for non-home pages (home page has it inside the banner) */}
        ...
        {activeStorylineId ? (
          ...
          <HomePageContent onStartChat={handleStartChat} onNavigate={(item) => {
            if (item === 'discover') return
            if (item === 'achievements') {
              window.dispatchEvent(new CustomEvent('chrona:open-achievements'))
              return
            }
            ...
            // Trigger sidebar's edit profile modal via custom event
            window.dispatchEvent(new CustomEvent('chrona:open-edit-profile'))
          }} onOpenMyPersonas={() => {
            // Navigate to discover and open personas modal via custom event
            handleSetActiveTab('home')
            window.dispatchEvent(new CustomEvent('chrona:open-my-personas'))
          }} />
        )}
