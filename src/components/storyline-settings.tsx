...
export function StorylineSettings({ storylineId, onClose, onUpdate }: StorylineSettingsProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="w-full max-w-4xl h-[90vh] bg-[#0d0718] rounded-xl border border-purple-500/20 shadow-2xl flex overflow-hidden">
        {/* Sidebar */}
        <div className="w-56 border-r border-purple-500/15 bg-[#0a0512] flex flex-col">
          <div className="p-4 border-b border-purple-500/15">
            <h2 className="font-bold text-lg text-purple-100">Server Settings</h2>
          </div>
          ...
          <div className="p-3 border-t border-purple-500/15">
            <button
              onClick={onClose}
              className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-sm text-purple-300/70 hover:text-purple-100 hover:bg-purple-500/10 transition-all"
            >
              <X className="w-4 h-4" />
              Close
            </button>
          ...
          <div className="flex-1 overflow-y-auto p-6">
            ...
            {activeTab === 'overview' && (
              <div className="space-y-6 max-w-xl">
                <h3 className="text-xl font-bold text-purple-100">Server Overview</h3>
                
                <div className="space-y-4">
                  <div>
                    <label className="text-sm text-purple-300 mb-1.5 block">Server Name</label>
                    <Input
                      value={name}
                      onChange={(e) => setName(e.target.value)}
                      disabled={!canChangeSettings}
                      className="bg-[#1a0f30] border-purple-500/30 text-purple-100"
                    />
                  </div>
                  
                  <div>
                    <label className="text-sm text-purple-300 mb-1.5 block">Short Description</label>
                    <Textarea
                      value={description}
                      ...
                      rows={2}
                      disabled={!canChangeSettings}
                      className="bg-[#1a0f30] border-purple-500/30 text-purple-100 resize-none"
                    />
                  </div>
                  
                  <div>
                    <label className="text-sm text-purple-300 mb-1.5 block">Lore / Backstory</label>
                    <Textarea
                      value={lore}
                  ...
                  <div>
                    <label className="text-sm text-purple-300 mb-1.5 block">Category</label>
                    <select
                      value={category}
                      onChange={(e) => setCategory(e.target.value)}
                      disabled={!canChangeSettings}
                      className="w-full px-3 py-2 rounded-lg bg-[#1a0f30] border border-purple-500/30 text-purple-100 focus:outline-none focus:ring-2 focus:ring-purple-500"
                    >
                  ...
                  <div className="flex items-center justify-between p-3 rounded-lg bg-[#1a0f30] border border-purple-500/20">
                    <div>
                      <p className="font-medium text-purple-100">Public Server</p>
                      <p className="text-xs text-purple-400/60">Anyone can find and join</p>
                    </div>
                    <button
                      onClick={() => setIsPublic(!isPublic)}
                      disabled={!canChangeSettings}
                      className={`w-12 h-6 rounded-full transition-all ${
                        isPublic ? 'bg-purple-500' : 'bg-purple-500/20'
                  ...
                  <div className="flex items-center justify-between p-3 rounded-lg bg-[#1a0f30] border border-purple-500/20">
                    <div>
                      <p className="font-medium text-purple-100">Require Approval</p>
                      <p className="text-xs text-purple-400/60">New members need approval to join</p>
                    </div>
                    <button
                      onClick={() => setRequireApproval(!requireApproval)}
                      disabled={!canChangeSettings}
                      className={`w-12 h-6 rounded-full transition-all ${
                        requireApproval ? 'bg-purple-500' : 'bg-purple-500/20'
                      } ${canChangeSettings ? 'cursor-pointer' : 'opacity-50 cursor-not-allowed'}`}
                    >
                      <div className={`w-5 h-5 rounded-full bg-white transition-all ${
                  ...
                  <div>
                    <label className="text-sm text-purple-300 mb-1.5 block">Member Cap (optional)</label>
                    <Input
                      type="number"
                      value={memberCap}
                      ...
                      className="bg-[#1a0f30] border-purple-500/30 text-purple-100"
                    />
                    <p className="text-xs text-purple-400/50 mt-1">Maximum number of members</p>
                  </div>
            ...
            {activeTab === 'appearance' && (
              <div className="space-y-6 max-w-xl">
                <h3 className="text-xl font-bold text-purple-100">Appearance</h3>
                
                <div className="space-y-4">
                  <div>
                    <label className="text-sm text-purple-300 mb-1.5 block">Accent Color</label>
                    <div className="flex gap-3">
                      <input
                        ...
                        disabled={!canChangeSettings}
                        className="w-12 h-10 rounded-lg cursor-pointer border border-purple-500/30"
                      />
                      <Input
                        value={accentColor}
                        onChange={(e) => setAccentColor(e.target.value)}
                        disabled={!canChangeSettings}
                        className="flex-1 bg-[#1a0f30] border-purple-500/30 text-purple-100"
                      />
                    </div>
                    <p className="text-xs text-purple-400/50 mt-1">This color appears on buttons and highlights</p>
                  </div>
                  ...
                  <div>
                    <label className="text-sm text-purple-300 mb-1.5 block">Welcome Message</label>
                    <Textarea
            ...
            {activeTab === 'channels' && (
              <div className="space-y-6">
                <h3 className="text-xl font-bold text-purple-100">Channels & Categories</h3>
                ...
                <div className="space-y-3">
                  ...
                  {storyline.categories.map(cat => (
                    <div key={cat.id} className="p-3 rounded-lg bg-[#1a0f30] border border-purple-500/20">
                      <div className="flex items-center justify-between mb-2">
                        <span className="font-medium text-purple-100">{cat.name}</span>
                        <span className="text-xs text-purple-400/60">{cat.channels.length} channels</span>
                      </div>
                      {cat.channels.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          {cat.channels.map(ch => (
                            <span key={ch.id} className="px-2 py-0.5 rounded text-xs bg-purple-500/10 text-purple-300">
                              # {ch.name}
                            </span>
                          ))}
                        </div>
                ...
                {canManageChannels && (
                  <div className="space-y-3">
                    <h4 className="text-sm font-semibold text-purple-300">Create Channel</h4>
                    <div className="flex gap-2">
                      <Input
                      ...
                      <select
                        ...
                        className="px-3 py-2 rounded-lg bg-[#1a0f30] border border-purple-500/30 text-purple-100"
                      >
            ...
            {activeTab === 'roles' && (
              <div className="space-y-6">
                <h3 className="text-xl font-bold text-purple-100">Roles</h3>
                
                <div className="space-y-2">
                  {storyline.roles.sort((a, b) => b.position - a.position).map(role => (
                    <div key={role.id} className="flex items-center gap-3 p-3 rounded-lg bg-[#1a0f30] border border-purple-500/20">
                      <div className="w-3 h-3 rounded-full" style={{ backgroundColor: role.color }} />
                      <span className="font-medium text-purple-100 flex-1">{role.name}</span>
                      {role.isAdmin && (
            ...
            {activeTab === 'invites' && (
              <div className="space-y-6">
                <h3 className="text-xl font-bold text-purple-100">Invite Links</h3>
                ...
                <div className="space-y-2">
                  {storyline.invites.length === 0 ? (
                    ...
                    storyline.invites.map(invite => (
                      <div key={invite.id} className="flex items-center gap-3 p-3 rounded-lg bg-[#1a0f30] border border-purple-500/20">
                ...
                {canInvite && (
                  <div className="p-4 rounded-lg bg-[#1a0f30] border border-purple-500/20 space-y-3">
                    <h4 className="font-medium text-purple-100">Create New Invite</h4>
                    <div className="flex gap-3">
                      <div>
                        <label className="text-xs text-purple-400/60 block mb-1">Max Uses</label>
                        <Input
                          type="number"
                          value={newInviteMaxUses}
                          onChange={(e) => setNewInviteMaxUses(e.target.value)}
                          placeholder="Unlimited"
                          className="w-28 bg-[#0d0718] border-purple-500/30 text-purple-100"
                        />
                      </div>
                      <div>
                        <label className="text-xs text-purple-400/60 block mb-1">Expires</label>
                        <select
                          value={newInviteExpiresIn}
                          onChange={(e) => setNewInviteExpiresIn(e.target.value)}
                          className="px-3 py-2 rounded-lg bg-[#0d0718] border border-purple-500/30 text-purple-100"
                        >
            ...
            {activeTab === 'members' && (
              <div className="space-y-6">
                <h3 className="text-xl font-bold text-purple-100">Members ({storyline.members.length})</h3>
                ...
                <div className="space-y-1">
                  {storyline.members.map(member => {
                    ...
                    return (
                      <div key={member.id} className="flex items-center gap-3 p-3 rounded-lg hover:bg-purple-500/5 transition-colors">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-purple-100 truncate">{member.user.username}</p>
            ...
            {activeTab === 'boosters' && canChangeSettings && (
              <div className="space-y-6">
                <h3 className="text-xl font-bold text-purple-100">Server Boosters</h3>
                
                {/* Tier Progress */}
                {boostStats && (
                  <div className="p-4 rounded-xl bg-[#1a0f30] border border-purple-500/20 space-y-3">
                ...
                {/* Boosters List */}
                <div className="space-y-3">
                  <h4 className="text-sm font-semibold text-purple-300">Active Boosters ({boosters.length})</h4>
                  ...
                  {isLoadingBoosters ? (
                    ...
                    <div className="space-y-2">
                      {boosters.map(booster => (
                        <div key={booster.id} className="flex items-center gap-3 p-3 rounded-lg bg-[#1a0f30] border border-purple-500/20">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-medium text-purple-100 truncate">{booster.persona.name}</p>
                            <p className="text-xs text-purple-400/60">@{booster.persona.username}</p>
                          </div>
                          <Badge 
                            variant="outline" 
                          ...
                          <div className="text-xs text-purple-400/50 text-right">
                            <div>Expires</div>
