...
export function StorylineAdvancedSearch({ onSearch, popularTags = [], featuredCategories = [], isLoading }: StorylineAdvancedSearchProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      {showAdvanced && (
        <div className="p-4 bg-gray-900/10 border border-white/15 rounded-xl space-y-4">
          {/* Search In Field */}
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              ...
              {STORYLINE_CATEGORIES.length > 7 && (
                <button
                  ...
                  className="px-2.5 py-1.5 rounded-lg text-xs font-medium bg-white/5 text-gray-400 hover:bg-white/10 flex items-center gap-1"
                >
                  {showAllCategories ? (
                    <>
          ...
          {featuredTags.length > 0 && (
            <div>
              <label className="text-xs font-medium text-gray-300 mb-2 flex items-center gap-1.5">
                ...
                Featured Tags
                <span className="text-gray-400/50 font-normal">- Most used by storylines</span>
              </label>
              <div className="flex flex-wrap gap-1.5">
                {displayedFeaturedTags.map(({ tag, count }) => (
                  ...
                  >
                    {tag}
                    <span className="text-gray-400/50 text-[10px]">({count})</span>
                  </button>
                ))}
                {featuredTags.length > 8 && (
                  <button
                    onClick={() => setShowAllFeaturedTags(!showAllFeaturedTags)}
                    className="px-2 py-1 rounded-full text-xs font-medium bg-white/5 text-gray-400 hover:bg-white/10 flex items-center gap-1 border border-white/10"
                  >
                    {showAllFeaturedTags ? (
                      <>
                        <ChevronUp className="w-3 h-3" />
            ...
            {filters.tags.length > 0 && (
              <div className="flex flex-wrap gap-1">
                {filters.tags.map(tag => (
                  <span
                    key={tag}
                    className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-white/10 text-gray-300 text-xs"
                  >
                    {tag}
