  ...
  return (
    <div className="bg-gray-900/20 rounded-xl p-4 border border-white/10">
      ...
      <div className="h-3 bg-gray-950/50 rounded-full overflow-hidden relative">
        <div className="absolute inset-0 bg-gradient-to-r from-gray-600/30 via-gray-400/30 to-gray-400/30" />
        <div 
          className="h-full bg-gradient-to-r from-white to-gray-300 rounded-full relative transition-all duration-300"
          style={{ width: `${value}%` }}
        >
...
function TextBlock({ content, className = '' }: { content: string; className?: string }) {
  return (
    <div className={`bg-white/5 rounded-xl p-4 border border-white/10 ${className}`}>
      <div className="text-white/90 text-sm leading-relaxed whitespace-pre-wrap markdown-content">
        <ReactMarkdown
          components={{
            ...
            blockquote: ({ children }) => (
              <blockquote className="border-l-2 border-white/30 pl-4 italic text-gray-300 my-2">
                {children}
              </blockquote>
            ),
...
export function MarketplacePersonaModal({ 
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div 
        className="w-full max-w-2xl max-h-[90vh] bg-gradient-to-b from-[#150a25] to-[#0a0312] rounded-2xl border border-white/15 shadow-2xl overflow-hidden flex flex-col"
      ...
      >
        ...
        <div className="p-4 border-b border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-400 text-sm">by @{listing.creator.username}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onClose}
              className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/5"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        <div className="px-4 py-3 border-b border-white/10 flex items-center gap-6">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm">
                <span className="font-semibold text-white">{listing.downloads + listing.purchaseCount}</span>
          ...
          {slotInfo && (
            <div className="flex items-center gap-2 ml-auto text-sm">
              ...
              <span className={`font-semibold ${hasSlots ? 'text-white' : 'text-red-400'}`}>
        ...
        {/* Tabs */}
        <div className="px-4 py-2 border-b border-white/10 flex gap-1">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              ...
              <div className="grid grid-cols-2 gap-3">
                {persona?.archetype && (
                  <div className="p-3 rounded-lg bg-white/5 border border-white/10">
                    <p className="text-xs text-gray-400/60">Archetype</p>
                    <p className="text-sm font-medium text-white">{persona.archetype}</p>
                  </div>
                )}
                {persona?.gender && (
                  <div className="p-3 rounded-lg bg-white/5 border border-white/10">
                    <p className="text-xs text-gray-400/60">Gender</p>
                    <p className="text-sm font-medium text-white">{persona.gender}</p>
                  </div>
                )}
                {persona?.age !== null && persona?.age !== undefined && (
                  <div className="p-3 rounded-lg bg-white/5 border border-white/10">
                    <p className="text-xs text-gray-400/60">Age</p>
                    <p className="text-sm font-medium text-white">{persona.age}</p>
                  </div>
                )}
                {persona?.species && (
                  <div className="p-3 rounded-lg bg-white/5 border border-white/10">
                    <p className="text-xs text-gray-400/60">Species</p>
                    <p className="text-sm font-medium text-white">{persona.species}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              {hobbies.length > 0 && (
                <Section title="Hobbies" icon={BookOpen}>
                  <TagGrid items={hobbies} color="gray" />
                </Section>
              )}
        ...
        {/* Footer Actions */}
        <div className="p-4 border-t border-white/10 bg-[#0a0610]">
          {/* Error Messages */}
          {purchaseError && (
          ...
          {listing.isOwner ? (
            <div className="flex items-center justify-center gap-2 py-3 px-4 rounded-lg bg-white/5 text-gray-300 font-medium border border-white/15">
