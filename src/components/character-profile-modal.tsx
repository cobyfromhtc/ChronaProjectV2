...
function TagChip({ label, color = 'gray' }: { label: string; color?: string }) {
  ...
  return (
    <span className={`px-2.5 py-1 rounded-lg text-xs border ${colors[color] || colors.gray}`}>
      {label}
    </span>
  )
}
...
function SpectrumBar({ label, value, leftLabel, rightLabel, icon }: { 
  ...
  return (
    <div className="bg-gray-900/20 rounded-xl p-4 border border-white/10">
      <div className="flex items-center gap-2 mb-3">
        {icon}
        <span className="text-sm font-medium text-gray-200">{label}</span>
      </div>
      <div className="flex justify-between text-xs text-gray-400/60 mb-2">
        <span>{leftLabel}</span>
        <span>{rightLabel}</span>
      </div>
      <div className="h-3 bg-gray-950/50 rounded-full overflow-hidden relative">
        <div className="absolute inset-0 bg-gradient-to-r from-gray-600/30 via-gray-500/30 to-gray-400/30" />
        <div 
          className="h-full bg-gradient-to-r from-gray-400 to-gray-300 rounded-full relative transition-all duration-300"
          style={{ width: `${value}%` }}
        >
          <div className="absolute right-0 top-1/2 -translate-y-1/2 w-3 h-3 bg-white rounded-full shadow-lg shadow-white/20" />
        </div>
      </div>
    </div>
  )
...
function TextBlock({ content, className = '' }: { content: string; className?: string }) {
  return (
    <div className={`bg-gray-900/15 rounded-xl p-4 border border-white/10 ${className}`}>
      <div className="text-gray-100/90 text-sm leading-relaxed whitespace-pre-wrap markdown-content">
        <ReactMarkdown
          components={{
            strong: ({ children }) => <strong className="font-bold text-white">{children}</strong>,
            em: ({ children }) => <em className="italic text-gray-200">{children}</em>,
            code: ({ children }) => (
              <code className="bg-white/10 px-1.5 py-0.5 rounded text-gray-200 text-xs font-mono">
                {children}
              </code>
            ),
            a: ({ href, children }) => (
              <a href={href} className="text-gray-400 underline underline-offset-2 hover:text-gray-300" target="_blank" rel="noopener noreferrer">
                {children}
              </a>
            ),
            blockquote: ({ children }) => (
              <blockquote className="border-l-2 border-white pl-4 italic text-gray-300 my-2">
                {children}
              </blockquote>
            ),
...
// Tag grid
function TagGrid({ items, color = 'gray' }: { items: string[] | string | null; color?: string }) {
...
export function CharacterProfileModal({ 
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'appearance' | 'personality' | 'attributes' | 'enhanced'>('overview')
  ...
  const tabs: { id: 'overview' | 'appearance' | 'personality' | 'attributes' | 'enhanced'; label: string; icon: React.ReactNode }[] = [
    ...
    { id: 'enhanced', label: 'Profile+', icon: <Zap className="w-4 h-4" /> },
  ]
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div 
        className="w-full max-w-2xl max-h-[90vh] bg-gradient-to-b from-black to-[#0a0a0a] rounded-2xl border border-white/20 shadow-2xl overflow-hidden flex flex-col"
      ...
      >
        ...
        <div className="p-4 border-b border-white/15 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Avatar className="w-12 h-12 border-2 border-white/20">
              ...
              <AvatarFallback className="bg-gradient-to-br from-gray-500 to-gray-600 text-white text-lg font-bold">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2">
                <span className="text-gray-400 text-sm">@{persona.username}</span>
                {persona.isOnline && (
        ...
        <div className="px-4 py-3 border-b border-white/15 flex items-center gap-x-6 gap-y-2 flex-wrap">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm">
                <span className="font-semibold text-gray-100">{followStatus?.followersCount ?? 0}</span>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm">
                <span className="font-semibold text-gray-100">{followStatus?.followingCount ?? 0}</span>
          ...
          <button
            ...
            className="flex-shrink-0 px-4 py-2 rounded-lg text-sm font-medium bg-white hover:bg-gray-100 text-black transition-all flex items-center gap-2"
          >
        ...
        {/* Tabs */}
        <div className="px-4 py-2 border-b border-white/15 flex gap-1">
          {tabs.map((tab) => (
            <button
              key={tab.id}
        ...
        <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar">
          ...
          {activeTab === 'attributes' && (
            <div className="space-y-4">
              {persona.mbtiType && (
                <div className="flex items-center gap-2 mb-2">
                  <span className="px-3 py-1 rounded-lg bg-white/10 text-gray-300 text-sm font-medium">
              ...
              {persona.hobbies && persona.hobbies.length > 0 && (
                <Section title="Hobbies" icon={BookOpen}>
                  <TagGrid items={persona.hobbies} color="gray" />
                </Section>
              )}
          ...
          {activeTab === 'enhanced' && (
            <div className="space-y-6">
              {/* Character Quote */}
              {persona.characterQuote && (
                <div className="p-4 rounded-xl bg-gradient-to-r from-white/5 to-gray-300/5 border border-white/10">
                  <p className="text-white/90 italic text-center text-lg leading-relaxed">
                    &quot;{persona.characterQuote}&quot;
                  </p>
                </div>
              )}
              ...
              {/* Psychology Section */}
              {(persona.psychologySurface || persona.psychologyBeneath) && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {persona.psychologySurface && (
                    <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                      <h4 className="text-sm font-semibold text-gray-200 mb-2 flex items-center gap-2">
                        <Eye className="w-4 h-4" />
                        Surface
                      </h4>
                      <p className="text-gray-100/80 text-sm leading-relaxed whitespace-pre-wrap">
                        {persona.psychologySurface}
                      </p>
                    </div>
                  )}
                  {persona.psychologyBeneath && (
                    <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                      <h4 className="text-sm font-semibold text-gray-200 mb-2 flex items-center gap-2">
              ...
              {/* Dossier */}
              {(persona.occupation || persona.status || persona.orientation || persona.height) && (
                <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {persona.occupation && (
                      <div className="flex justify-between items-center py-1.5 border-b border-white/10">
                        <span className="text-xs text-gray-400/60 uppercase tracking-wider">Occupation</span>
                        <span className="text-sm text-gray-100">{persona.occupation}</span>
                      </div>
                    )}
                    {persona.status && (
                      <div className="flex justify-between items-center py-1.5 border-b border-white/10">
                        <span className="text-xs text-gray-400/60 uppercase tracking-wider">Status</span>
                        <span className="text-sm text-gray-100">{persona.status}</span>
                      </div>
                    )}
                    {persona.orientation && (
                      <div className="flex justify-between items-center py-1.5 border-b border-white/10">
                        <span className="text-xs text-gray-400/60 uppercase tracking-wider">Orientation</span>
                        <span className="text-sm text-gray-100">{persona.orientation}</span>
                      </div>
                    )}
                    {persona.height && (
                      <div className="flex justify-between items-center py-1.5 border-b border-white/10">
                        <span className="text-xs text-gray-400/60 uppercase tracking-wider">Height</span>
                        <span className="text-sm text-gray-100">{persona.height}</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {/* Dialogue Log */}
              {persona.dialogueLog && persona.dialogueLog.length > 0 && (
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {persona.dialogueLog.map((dialogue, i) => (
                      <div key={i} className="p-3 rounded-lg bg-white/5 border border-white/10">
                        <div className="flex items-center gap-2 mb-1.5">
                          <span className="px-2 py-0.5 rounded text-xs font-medium bg-white/10 text-gray-300 uppercase">
                            {dialogue.type}
                          </span>
                          {dialogue.mood && (
                        ...
                        <p className="text-gray-100/80 text-sm italic leading-relaxed">
                          &quot;{dialogue.content}&quot;
                        </p>
                      </div>
                    ))}
              ...
              {/* Scenarios */}
              {persona.characterScenarios && persona.characterScenarios.length > 0 && (
                <div className="space-y-3">
                  <h4 className="text-sm font-semibold text-gray-200 flex items-center gap-2">
                  ...
                  <div className="grid gap-2">
                    {persona.characterScenarios.map((scenario, i) => (
                      <div key={i} className="p-3 rounded-lg bg-white/5 border border-white/10">
                        <div className="flex items-start gap-2">
                          <span className="flex-shrink-0 w-6 h-6 rounded-full bg-white/10 flex items-center justify-center text-xs text-gray-300 font-medium">
                            {i + 1}
                          </span>
                          <div>
                            <h5 className="text-sm font-medium text-gray-100">{scenario.title}</h5>
                            <p className="text-xs text-gray-300/60 mt-0.5">{scenario.description}</p>
                          </div>
                        </div>
                      </div>
                    ))}
              ...
              {/* Gallery */}
              {persona.galleryUrls && persona.galleryUrls.length > 0 && (
                <div className="space-y-3">
                  <h4 className="text-sm font-semibold text-gray-200 flex items-center gap-2">
                    <Sparkles className="w-4 h-4" />
                    Gallery
              ...
              {/* Empty state */}
              {!persona.characterQuote && !persona.psychologySurface && !persona.psychologyBeneath && 
               !persona.occupation && !persona.status && !persona.orientation && !persona.height &&
               (!persona.dialogueLog || persona.dialogueLog.length === 0) && 
               (!persona.characterScenarios || persona.characterScenarios.length === 0) &&
               (!persona.galleryUrls || persona.galleryUrls.length === 0) && (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500">No enhanced profile content yet</p>
                </div>
              )}
            </div>
          )}
