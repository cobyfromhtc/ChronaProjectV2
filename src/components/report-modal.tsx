...
export function ReportModal({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="bg-[#0d0718] border-white/15 text-white max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="text-gray-400/60">
            {reportedName && (
              <span>
                Reporting: <span className="text-gray-200 font-medium">{reportedName}</span>
              </span>
            )}
          </DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {messagePreview && (
              <div className="p-3 rounded-lg bg-white/5 border border-white/15">
                <p className="text-xs text-gray-400/60 mb-1">Message:</p>
                <p className="text-sm text-gray-200 line-clamp-3">"{messagePreview}"</p>
              </div>
            )}
                  ...
                  <button
                    ...
                    className={`px-3 py-2 rounded-lg text-sm text-left transition-all ${
                      reason === r.value
                        ...
                        : 'bg-white/5 border border-white/15 text-gray-300 hover:bg-white/5'
                    }`}
                  >
                    {r.label}
                  </button>
            ...
            <div className="space-y-2">
              <label className="text-sm text-gray-300 font-medium">
                Additional Details
                <span className="text-gray-400/60 font-normal"> (optional)</span>
              </label>
              <textarea
                value={details}
                onChange={(e) => setDetails(e.target.value)}
                placeholder="Provide any additional context that might help our moderators..."
                className="w-full h-24 px-3 py-2 rounded-lg bg-white/5 border border-white/15 text-white placeholder:text-gray-400/40 focus:border-white focus:outline-none resize-none text-sm"
                maxLength={1000}
              />
              <p className="text-xs text-gray-400/40 text-right">{details.length}/1000</p>
            </div>
