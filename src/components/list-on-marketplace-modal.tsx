        ...
        <DialogHeader className="p-6 pb-4 border-b border-white/10 flex-shrink-0">
          <div className="flex items-center gap-4">
            <Avatar className="w-16 h-16 border-2 border-white/20">
              ...
              <AvatarFallback className="bg-gradient-to-br from-white to-gray-300 text-white text-xl font-semibold">
            ...
            <div>
              ...
              <DialogDescription className="text-gray-400/60 mt-1">
                Share <span className="text-gray-300 font-medium">{persona.name}</span> with the community
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="p-6 space-y-5 overflow-y-auto flex-1 min-h-0">
          ...
          {success && (
            <div className="flex flex-col items-center justify-center py-8">
              ...
              <h3 className="text-lg font-bold text-white">Listed Successfully!</h3>
              <p className="text-sm text-gray-400/60 mt-1">Your persona is now on the marketplace</p>
            </div>
          )}
          ...
          {!success && (
            <>
              ...
              <div className="space-y-2">
                <Label className="text-gray-200/80 flex items-center gap-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    placeholder="0"
                    className="h-12 text-2xl font-bold text-center bg-white/5 border-white/15 text-white placeholder:text-gray-400/40 focus:border-white/30"
                  />
                  <span className="absolute right-4 top-1/2 -translate-y-1/2 text-sm text-gray-400/60">
                    Max: 430
                  </span>
                </div>
                {numPrice > 0 && (
                ...
                {numPrice === 0 && (
                  <p className="text-xs text-gray-400/50">Free personas get more downloads!</p>
                )}
              </div>
              
              {/* Notification Toggle */}
              <div className="space-y-2">
                <div className="flex items-center justify-between p-3 rounded-lg bg-white/5 border border-white/15">
                  <div className="flex items-center gap-3">
                    {notifyOnPurchase ? (
                      <Bell className="w-5 h-5 text-gray-400" />
                    ) : (
                      <BellOff className="w-5 h-5 text-gray-400/50" />
                    )}
                    <div>
                      <Label className="text-gray-200/80 cursor-pointer">
              ...
              {/* Description */}
              <div className="space-y-2">
                <Label className="text-gray-200/80">Description</Label>
                <Textarea
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  placeholder={persona.description || "Describe your persona for the marketplace..."}
                  className="min-h-[80px] bg-white/5 border-white/15 text-white placeholder:text-gray-400/40 resize-none"
                  maxLength={500}
                />
                <p className="text-xs text-gray-400/50 text-right">{description.length}/500</p>
              </div>
              ...
              <div className="space-y-2">
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                        selectedTags.includes(tag)
                          ...
                          : 'bg-white/5 text-gray-300/70 border border-white/15 hover:bg-white/5'
                      }`}
                    >
                      {tag}
                    </button>
                ...
                {selectedTags.length === 0 && (
                  <p className="text-xs text-gray-400/50">Select at least one archetype</p>
                )}
              </div>
        ...
        {!success && (
          <div className="p-6 pt-4 flex gap-3 border-t border-white/10 flex-shrink-0 bg-[#12091f]">
            ...
            <Button
              ...
              className="flex-1 bg-gradient-to-r from-gray-700 to-gray-500 hover:from-white hover:to-gray-400 text-white"
            >
              {isSubmitting ? (
                <>
