...
import { 
  ...
  ChevronRight, UserPlus, Settings, LogIn, X, Heart, Edit2, Wallet, Shield
} from 'lucide-react'
...
interface SidebarProps {
  activeTab: 'home' | 'friends' | 'storylines' | 'chat' | 'wallet' | 'admin'
  activeStorylineId: string | null
  onSelectTab: (tab: 'home' | 'friends' | 'storylines' | 'chat' | 'wallet' | 'admin') => void
...
export function Sidebar({
  ...
  const handleRevealSecurityKey = async () => {
    ...
    try {
      const response = await fetch('/api/auth/security-key', {
        method: 'POST',
      })
  ...
  return (
    <div className="w-56 flex flex-col h-full flex-shrink-0 relative bg-gradient-to-b from-[#0d0718] to-[#090517] border-r border-purple-500/15">
      {/* Decorative top glow */}
      <div className="absolute top-0 left-0 right-0 h-32 bg-gradient-to-b from-purple-500/10 to-transparent pointer-events-none z-0" />
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            <img 
              src="/logo.png" 
              alt="Chrona" 
              className="w-10 h-10 rounded-xl shadow-lg shadow-purple-500/25 object-cover"
            />
            <Sparkles className="absolute -top-1 -right-1 w-4 h-4 text-fuchsia-400" />
          </div>
          <div>
            <h1 className="font-bold text-lg bg-gradient-to-r from-purple-400 to-fuchsia-400 bg-clip-text text-transparent">Chrona</h1>
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden relative z-10">
        ...
        {storylines.length === 0 && (
          <div className="px-3 mt-3 flex-shrink-0">
            <button
              onClick={() => onSelectTab('storylines')}
              className="w-full p-3 rounded-lg border border-dashed border-purple-500/30 flex flex-col items-center justify-center gap-1 text-purple-400/60 hover:text-purple-300 hover:border-purple-500/50 transition-all"
            >
      ...
      <div className="persona-user-panel relative z-20 flex-shrink-0" ref={profileMenuRef}>
        <button 
          onClick={() => { setIsProfileMenuOpen((prev) => !prev); setEditUsername(user?.username || '') }} 
          className="w-full flex items-center gap-3 p-2 rounded-xl hover:bg-purple-500/10 transition-colors"
        >
          <div className="relative">
            ...
            <span className="absolute bottom-0 right-0 w-3 h-3 bg-emerald-400 rounded-full border-2 border-[#0d0718] animate-pulse" />
          </div>
          <div className="flex-1 min-w-0 text-left">
        ...
        {isEditProfileOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div className="w-full max-w-md rounded-xl bg-[#120a24] border border-purple-500/30 p-5 relative">
              ...
              <h3 className="text-lg font-semibold text-purple-100 mb-4">Edit Profile</h3>
              ...
              >
                <label className="block text-xs text-purple-300 mb-1">Username</label>
                <input
                  type="text"
                  value={editUsername}
                  onChange={(e) => setEditUsername(e.target.value)}
                  className="w-full px-3 py-2 rounded-lg bg-[#1e0f3a] border border-purple-500/30 text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                />
                <div className="mt-4 flex justify-end gap-2">
                  <button
                    type="button"
                    onClick={() => setIsEditProfileOpen(false)}
                    className="px-3 py-2 rounded-lg text-sm border border-purple-500/30 text-purple-300 hover:bg-purple-500/10"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="px-3 py-2 rounded-lg bg-gradient-to-r from-purple-600 to-fuchsia-600 text-white text-sm hover:opacity-90"
                  >
                    Save
                  </button>
                </div>
                ...
                <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-3 mb-3">
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-xs text-amber-200/80">
                      This will generate a <strong>new</strong> security key. Your old key will no longer work.
                    </p>
                  </div>
                </div>
                
                <button
                  onClick={() => setShowRevealKeyConfirm(true)}
                  className="w-full py-2.5 rounded-lg border border-amber-500/30 text-amber-300 text-sm font-medium hover:bg-amber-500/10 transition-colors flex items-center justify-center gap-2"
                >
                  <Crown className="w-4 h-4" />
                  Reveal Account Security Key
                </button>
              </div>
            </div>
          </div>
