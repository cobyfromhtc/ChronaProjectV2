...
const categoryColors: Record<string, { bg: string; text: string; border: string; gradient: string }> = {
  'Fantasy': { 
    ...
    gradient: 'from-purple-600/30 via-fuchsia-500/20 to-violet-600/30'
  },
  'Sci-Fi': { 
    bg: 'bg-cyan-500/20', 
    text: 'text-cyan-300', 
  ...
  'Drama': { 
    ...
    gradient: 'from-violet-600/30 via-purple-500/20 to-violet-600/30'
  },
  'Action': { 
    bg: 'bg-orange-500/20', 
  ...
  'Supernatural': { 
    ...
    gradient: 'from-indigo-600/30 via-violet-500/20 to-indigo-600/30'
  },
  'Other': { 
    bg: 'bg-gray-500/20', 
    text: 'text-gray-300', 
...
export function StorylineServerCard({ server, onClick, variant = 'default' }: StorylineServerCardProps) {
  ...
  if (variant === 'compact') {
    return (
      <button
        onClick={onClick}
        className="w-full flex items-center gap-3 p-2 rounded-xl hover:bg-purple-500/10 transition-all group text-left"
      >
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-purple-500 to-fuchsia-500 flex items-center justify-center overflow-hidden flex-shrink-0 ring-2 ring-purple-500/20 group-hover:ring-purple-500/40 transition-all">
          {iconImage ? (
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-purple-100 truncate">{server.name}</p>
          <p className="text-xs text-purple-400/60">{memberCount} members</p>
        </div>
  ...
  if (variant === 'featured') {
    return (
      <div
        onClick={onClick}
        className="relative rounded-2xl overflow-hidden cursor-pointer group border border-purple-500/20 hover:border-purple-500/40 transition-all"
      >
        {/* Banner Background */}
        <div className="h-48 relative">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-[#0d0718] via-[#0d0718]/60 to-transparent" />
        ...
        <div className="p-4 relative -mt-12">
          {/* Server Icon */}
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-purple-500 to-fuchsia-500 flex items-center justify-center overflow-hidden ring-4 ring-[#0d0718] shadow-xl mb-3">
            {iconImage ? (
              <img src={iconImage} alt={server.name} className="w-full h-full object-cover" />
            ) : (
          ...
          <p className="text-sm text-purple-300/70 line-clamp-2 mb-3">{server.description}</p>
  ...
  return (
    <div
      onClick={onClick}
      className="rounded-xl border border-purple-500/20 overflow-hidden cursor-pointer group hover:border-purple-500/40 hover:shadow-lg hover:shadow-purple-500/5 transition-all bg-[#1a1230]/80"
    >
      {/* Banner */}
      <div className="h-28 relative overflow-hidden">
        ...
        <div className="absolute bottom-3 right-3">
          {server.isPublic === false ? (
            <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-black/40 backdrop-blur-sm text-xs text-purple-300/80">
        ...
        <div className="absolute -bottom-6 left-4">
          <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-purple-500 to-fuchsia-500 flex items-center justify-center overflow-hidden ring-4 ring-[#1a1230] shadow-lg">
            {iconImage ? (
              <img src={iconImage} alt={server.name} className="w-full h-full object-cover" />
            ) : (
      ...
      <div className="p-4 pt-9">
        <div className="flex items-start justify-between gap-2 mb-2">
          <h3 className="font-semibold text-purple-100 group-hover:text-white transition-colors truncate">
            {server.name}
          </h3>
          {server.isMember && (
        ...
        <p className="text-sm text-purple-300/60 line-clamp-2 mb-3">
          {server.description}
        </p>
        ...
        {server.tags && (
          <div className="flex flex-wrap gap-1.5 mb-3">
            {server.tags.split(',').slice(0, 3).map((tag, index) => (
              <span 
                key={index}
                className="px-2 py-0.5 rounded-full text-xs bg-purple-500/10 text-purple-400/80 border border-purple-500/20"
              >
                {tag.trim()}
              </span>
            ))}
        ...
        <div className="flex items-center justify-between pt-3 border-t border-purple-500/10">
          ...
          {server.owner && (
            <div className="flex items-center gap-1.5">
              <Avatar className="w-5 h-5 border border-purple-500/30">
                <AvatarImage src={server.owner.avatar || server.owner.avatarUrl || undefined} />
                <AvatarFallback className="bg-purple-500/30 text-purple-200 text-[8px]">
                  {(server.owner.username || server.owner.name || 'U').charAt(0).toUpperCase()}
                </AvatarFallback>
              </Avatar>
              <span className="text-xs text-purple-400/50">@{server.owner.username || server.owner.name}</span>
            </div>
          )}
        </div>
      </div>
